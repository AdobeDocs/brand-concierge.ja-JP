---
title: コンシェルジュの作成 – ビデオ
description: Adobe Brand Conciergeを設定して、ブランドの声やガイドラインを活用して、サイトでパーソナライズされた会話型の顧客体験を提供する方法を説明します。
topic: Personalization,Integrations
role: User
level: Beginner
doc-type: Tutorial
duration: 213
last-substantial-update: 2025-10-31T00:00:00.000Z
jira: KT-19504
TQID: https://experienceleague.adobe.com/RDjW-BGiusLzV4wcUQpJ7kB6VxE01H-a6iaDlwbGofc
product_v2:
  - id: b6ee73fe-bdc6-47d9-99a2-80194514dd40
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2:
  - id: a09a5a04-e30b-4d55-b031-38e6f5ec86db
  - id: bce87dde-a4ab-44c9-8a18-ad66e4ddb377
  - id: e0eb8757-182f-49f3-94a4-1587d16f5094
source-git-commit: b64a90b1ad70162ca26967297254c5b83d3da39e
workflow-type: tm+mt
source-wordcount: 138
ht-degree: 0%

---

# 最初のコンシェルジュを作成

Adobeを初めて導入し、パーソナライズされた会話体験を提供する方法を学びましょう。 コンシェルジュの名前を付ける方法、役に立つコンテンツを追加する方法、コンシェルジュの対応を選択する方法、自社のブランドに合わせてコンシェルジュの声を聞く方法などについて解説します。 準備ができたら、テストして、デスクトップとモバイルでの動作を確認できます。

## この動画は誰のためのものでしょうか？

* デジタルExperience Manager
* マーケティング技術者
* Adobe Customer Experience Designer

## ビデオコンテンツ

* Adobe Brand Conciergeの設定方法
* コンテンツソースを連携させて正確に回答する方法
* ブランドボイスをカスタマイズし、エクスペリエンスをテストする方法

>[!VIDEO](https://video.tv.adobe.com/v/3476522/?captions=jpn&learn=on)

ドキュメントについては、[Brand Concierge ヘルプ &#x200B;](../documentation/overview.md)を参照してください。

