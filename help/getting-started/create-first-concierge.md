---
title: コンシェルジュの作成 – ビデオ
description: Adobe Brand Conciergeを設定して、ブランドの声やガイドラインを活用して、サイトでパーソナライズされた会話型の顧客体験を提供する方法を説明します。
topic: Personalization,Integrations
role: User
level: Beginner
doc-type: Tutorial
duration: 372
last-substantial-update: 2026-08-20
jira: KT-19504
TQID: https://experienceleague.adobe.com/RDjW-BGiusLzV4wcUQpJ7kB6VxE01H-a6iaDlwbGofc
product_v2:
  - id: b6ee73fe-bdc6-47d9-99a2-80194514dd40
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2:
  - id: a09a5a04-e30b-4d55-b031-38e6f5ec86db
  - id: bce87dde-a4ab-44c9-8a18-ad66e4ddb377
  - id: e0eb8757-182f-49f3-94a4-1587d16f5094
source-git-commit: 5e6249cc834071ae1f330278a4150fe6140da758
workflow-type: tm+mt
source-wordcount: 160
ht-degree: 0%

---

# 最初のコンシェルジュを作成

わずか数分で最初のAdobe Brand Conciergeを設定する方法について説明します。 web サイトのURLを入力すると、Brand Conciergeがスターターとなるブランドプロファイルと手順をサイト独自のコンテンツから自動的に構築し、保存する前に確認して調整することができます。 コンシェルジュを構築したら、デスクトップとモバイルの両方でサンプルの質問を使用してテストし、独自のサイトコンテンツを使用してどのように対応するのかを確認します。

## この動画は誰のためのものでしょうか？

* デジタルExperience Manager
* マーケティング技術者
* Adobe Customer Experience Designer

## ビデオコンテンツ

* web サイトのURLを入力することで、ブランドプロファイルとその手順を自動的に入力する方法
* 保存する前にブランドプロファイル、手順、スタータープロンプトを確認し、調整する方法
* デスクトップとモバイルでサンプルの質問を使用してコンシェルジュをテストする方法

>[!VIDEO](https://video.tv.adobe.com/v/3476521?learn=on)

ドキュメントについては、[Brand Concierge ヘルプ &#x200B;](../documentation/overview.md)を参照してください。

