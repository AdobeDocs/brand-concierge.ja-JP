...
### Brand Conciergeは、web サイトと同じドメインに存在する必要がありますか？

いいえ。 Brand Conciergeはドメインに依存しません。インスタンスは、デプロイされるサイトのドメインと一致する必要はありません。
...
はい。 Brand Conciergeは、データレジデンシーを構築しているため、会話中に収集された個人データ（名前、メールアドレス、郵送先住所など）は、適用される地域の規制に沿ってローカルのままになります。
...
### 設定した以外の組み込みの安全ガードレールはありますか？

はい。 コンテンツや冒涜的な表現のフィルターに加えて、安全に関するガードレールは、ユーザー自身の設定に関係なく、システムレベルで適用されます。 自社のブランドや業界固有のものを、設定した説明書に従って階層化できます。
...
### ナレッジベースの最初のQ&amp;A ペアはどのように生成されますか？

自動生成モードでは、Brand Conciergeが独自のサイトから抽出したコンテンツを利用して、最初のQ&amp;A ペアを生成します。 その後、生成されたQ&amp;Aを、公開前に自身の専門知識にもとづいてレビューし、編集します。

### サイトの変更に伴い、ナレッジベースは最新の状態を維持しますか？

はい。 コンテンツは、定期的なスケジュールで、または手動でいつでも更新できるため、削除されたコンテンツや古いコンテンツ（壊れたリンク、廃止されたページ、価格変更、廃盤になった製品など）をナレッジベースから削除することができます。
...
### Brand Conciergeは訪問者にどの程度迅速に対応しますか？
...
### Brand Conciergeは、コンテンツとデータに対して検索拡張（RAG）システムとして機能しますか？

はい。 Brand Conciergeは、取り込んだナレッジからRAG レイヤーを構築します。 ナレッジソースがURLまたはサイトマップから作成されると、Brand Conciergeはコンテンツをスクレイピングし、バックグラウンドでベクターデータベースを構築します。
...
### Adobe Commerceとの統合はどのように機能し、Commerce as PaaSをサポートしていますか？

Brand Conciergeは、Adobe Commerce MCP （モデルコンテキストプロトコル）を介してAdobe Commerceに接続します。 これには、Commerce as PaaSを運用しているお客様に対するネイティブサポートが含まれ、セルフサービスのさまざまな基本的なユースケースをカバーしています。より複雑なバリエーションやカスタムバリエーションには、バックエンドの作業が追加で必要になる場合があります。

### Brand Conciergeは、カタログの閲覧やレコメンデーションだけでなく、ショッピングもサポートしていますか？

現在、Brand Conciergeは商品の検索をサポートしています。 カートに商品を追加したり、チェックアウトしたりするなど、より詳細なコマースアクションは、今後の機能強化で計画されています。
...
### Brand Conciergeのカスタムイベントを自分のAnalyticsで追跡できますか？

はい。 同社のコールバック機能は、顧客がコールバックにアクセスしたときにトリガーを発するクライアント側のコールバック機能です。トリガーやトラッキングを選択するのは、あなた自身の判断です。
